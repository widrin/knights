...
func (d *DataRow) GetUInt32(fieldName string) uint32 {
	v, ok := d.Fields[fieldName]
	if ok {
...
func QueryWithoutPrepare(conn *sql.DB, strsql string, args ...interface{}) ([]*DataRow, error) {
	fullSql, err0 := sqlbuilder.MySQL.Interpolate(strsql, args)
	if err0 != nil {
		return nil, err0
	}
	rows, err := conn.QueryContext(context.Background(), fullSql)
	if err != nil {
		return nil, err
	}
	defer func() {
...
func ExecuteScalarint64(_db *sql.DB, sql string, columnIndex int) int64 {
	dt, err := QueryDataTable(_db, sql)
	if err != nil {
		log.Error("ExecuteScalarStr:aaaa:%v", err)
		return 0
	}
	for _, v := range dt.DataRows {
