...
var DefaultClient = &http.Client{Timeout: 10 * time.Second, Transport: DefaultTransport}
...
func New() *SuperAgent {
	...
	//client := cliPool.Get().(*http.Client)
	//client.Jar = jar
...
func (s *SuperAgent) SetClient(client *http.Client) *SuperAgent {
	s.client = client
	return s
}
...
func (s *SuperAgent) getResponseBytes() (Response, []byte, []error) {
	...
	if s.Debug {
		...
		if nil != err {
			...
			log.Debug("HTTP Response:%s", string(dump))
		}
	}
