...
func AddDateInt(date int32, addday int32) int32 {
	tt := GetTimeByDateInt(date)
	tt = tt.Add(time.Hour * 24 * time.Duration(addday))
	return int32(tt.Year()*10000 + int(tt.Month())*100 + tt.Day())
}
...
func SubDaysTimeStamp(timeStamp1 int64, timeStamp2 int64) int32 {
...
func GetNowDayTimestamp(timestamp int64) int64 {
	return timestamp - GetNowSceond(timestamp)
}
...
func HourAndMinuteToSecond(t string) int64 {
	tt := strings.Split(t, ":")
	return convert.ToInt64(tt[0])*3600 + convert.ToInt64(tt[1])*60
}
