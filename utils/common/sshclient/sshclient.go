...
func DialWithKey(addr, user, keyfile string) (*Client, error) {
	...
	return DialWithKeyValue(addr, user, key)
}
func DialWithKeyValue(addr, user string, keyValue []byte) (*Client, error) {

	signer, err := ssh.ParsePrivateKey(keyValue)
	if err != nil {
		return nil, err
	}
...
func (rs *RemoteScript) SmartOutput() ([]byte, error) {
	...
	err := rs.Run()
	if err != nil {
		r := append(stdout.Bytes(), stderr.Bytes()...)
		return r, err
	}
