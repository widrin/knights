...
func Init(redisdbAddr, pass string, isJq bool, db int32) {
...
func InitClient(cfg *_redis.Options) {
	log.Release("redis init ")
	Redis = _redis.NewClient(cfg)
}
func GetKeyExists(key string) (bool, error) {
	ret := Redis.Exists(ctx, key)
	if ret.Err() != nil {
...
func IncrbyFieldNum(key string, val int64) error {
	ret := Redis.IncrBy(ctx, key, val)
	return ret.Err()
}

func SetIncrbyHashFiledVal(key string, field interface{}, val interface{}) (int64, error) {
	ret := Redis.HIncrBy(ctx, key, convert.ToString(field), convert.ToInt64(val))
	if ret.Err() != nil {
...
func GetIntVal64(key string) int64 {
	ret, err := Redis.Get(ctx, key).Int64()
...
func GetHashKeyExists(key string, field interface{}) (bool, error) {
	ret := Redis.HExists(ctx, key, convert.ToString(field))
	if ret.Err() != nil {
		log.Error("GetKeyExists:%v", ret.Err())
		return false, ret.Err()
	}
...
func SetHashFiledVal(key string, field interface{}, val interface{}) error {
	ret := Redis.HSet(ctx, key, convert.ToString(field), val)
	if ret.Err() != nil {
		log.Error("SetHashFiledVal:%v>>>%v>>>%v>>>%v", key, field, val, ret.Err())
	}
	return ret.Err()
...
func HsetNXFiledVal(key string, field interface{}, val interface{}) error {
	ret := Redis.HSetNX(ctx, key, convert.ToString(field), convert.ToString(val))
	if ret.Err() != nil {
...
func GetHashStringVal(key string, field interface{}) string {
	ret := Redis.HGet(ctx, key, convert.ToString(field))
	if ret.Err() != nil && ret.Err().Error() != RedisNil {
...
func GetHashIntVal(key string, field interface{}) (int32, error) {
	...
	ret, err := Redis.HGet(ctx, key, convert.ToString(field)).Int()
	if err != nil && err.Error() != RedisNil {
...
func GetHashIntVal64(key string, field interface{}) int64 {
	ret, err := Redis.HGet(ctx, key, convert.ToString(field)).Int64()
	if err != nil && err.Error() != RedisNil {
...
func IncrbyHashIntFieldNum(key string, field interface{}, val int32) error {
	...
	if n < 0 {
		...
		Redis.HIncrBy(ctx, key, convert.ToString(field), -convert.ToInt64(val))
...
func IncrbyHashInt64FieldNum(key string, field string, val int64) error {
	ret := Redis.HIncrBy(ctx, key, convert.ToString(field), val)
	if ret.Err() != nil {
	...
	if n < 0 {
		...
		Redis.HIncrBy(ctx, key, convert.ToString(field), -val)
		return errors.New("数量不足")
	}
	return ret.Err()
}
func DelHashKey(key string, field interface{}) error {
	ret := Redis.HDel(ctx, key, convert.ToString(field))
	if ret.Err() != nil {
...
func SetEx(key string, second int32, value interface{}) {
	ret := Redis.Set(ctx, key, value, time.Duration(second)*time.Second)
...
func EXPIRE(key string, second int32) {
	ret := Redis.Expire(ctx, key, time.Duration(second)*time.Second)
...
func GetHashHMGet(key string, fields []interface{}) map[string]interface{} {
	info := make(map[string]interface{})
	field := make([]string, 0, len(fields))
	for _, v := range fields {
		field = append(field, convert.ToString(v))
	}
	ret := Redis.HMGet(ctx, key, field...)
	if ret.Err() != nil {
		log.Error("GetHashHmGet:%v>>>%v", key, ret.Err())
		return info
	}
	for i, v := range ret.Val() {
		key := field[i]
		info[key] = v
	}
...
func SetHashHMSet(key string, fieldValues []interface{}) {
	ret := Redis.HMSet(ctx, key, fieldValues)
	if ret.Err() != nil {
		log.Error("SetHashHMSet:%v>>>%v>>>%v", key, fieldValues, ret.Err())
	}
}

func GetMGet(keys []string) map[string]interface{} {
	info := make(map[string]interface{})
	ret := Redis.MGet(ctx, keys...)
	if ret.Err() != nil {
		log.Error("GetMGet:%v>>>%v", keys, ret.Err())
		return info
	}
	for i, v := range ret.Val() {
		key := keys[i]
...
func SetMSet(values []interface{}) {
	ret := Redis.MSet(ctx, values)
	if ret.Err() != nil {
		log.Error("SetMSet:%v>>>%v", values, ret.Err())
	}
}

func GetMutiHashPipeline(keys []string, fields []string) []map[string]interface{} {
	info := make([]map[string]interface{}, 0)
	pipe := Redis.Pipeline()
	for _, key := range keys {
		pipe.HMGet(ctx, key, fields...)
	}
	cmd, err := pipe.Exec(ctx)
	if err != nil {
		log.Error("Pipeline:%v>>>%v", err)
		return info
	}
	for _, cmder := range cmd {
		info1 := make(map[string]interface{})
		cmderSlice, ok := cmder.(*_redis.SliceCmd)
		if ok {
			for i1, v := range cmderSlice.Val() {
				info1[fields[i1]] = v
			}
		}
